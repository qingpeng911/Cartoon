...
<div>
	...
	<div>
		<div class="index" v-show="!loading">
			<div class="bookshelf">
				<!--header-->
				<headbar leftName="收藏" rightName="历史" leftUrl="#/index/bookshelf/collection" leftSelected="true" rightUrl="#/index/bookshelf/history" :rightButtonType="rightButtonType" v-on:childMethod="switchEdit"></headbar>
				<div class="bookshelfList">
          <div class="bookshelfListMain" v-for="(item,index) in list" v-on:click="itemClick(item,index)">
            <div class="bookshelfListChoiceBut editDiv bookshelfListChoiceButSelect" v-show="edit" style="height: 0px; top: 2px; right: 2px;">
            ...
            <div class="bookshelfListMainImg">
            ...
            <div class="bookshelfListMainText">
              ...
              <div class="bookshelfListMainDes">
                <span class="lastSort">{{item.curNum}}</span>话/{{item.allNum}}话
              </div>
            </div>
          </div>
				</div>
...
export default {
  ...
  methods: {
    getList() {
      ...
      api.getCollections({pageSize: 10, page: this.page}).then(res => {
    ...
    handleDel() {
      ...
      if (this.delNum.length == 0) {
        ...
        for (var i = 0; i < len; i++) {
          ...
          api.deleteCollection(cobj.cartoonId).then(res => {
